...
public class SocketServer implements Runnable {
	 ...
	 *  @param true is to identify the messageList to server
	*/
    public static LinkedList messageList = new LinkedList(true);	
			
	public SocketServer(ServerFrame frame) {

		clients = new ServerThread[35]; //set 35 as the total clients inside the chat room to match students in class
		gui = frame;
		...
		try {
			...
			gui.jTextArea1.append("Cannot bind to port : " + port + "\nRetrying");
			gui.RetryStart(0);
		}
	}
		...
		if (msg.content.equals(".bye")) {
			...
			if (msg.type.equals("login")) {
				if (findUserThread(msg.sender) == null) {
						...
						// Creates the client's messageList
						LinkedList clientMessageList = messageList.getCurrentUserMessages(msg.sender);
						for (int x=0; x< clientMessageList.size(); x++)	{
							Message newMSG = clientMessageList.getMessageByIndex(x);
							clients[findUser(ID)].send(new Message(newMSG.time,
									newMSG.type,
									newMSG.sender,
									newMSG.content,
									msg.sender, // Client which has to receive its client message history
									newMSG.recipient)); // ACTUAL recipient of each message sent by that client
						}
