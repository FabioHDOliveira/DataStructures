...
public class LinkedList {
	...
	*/
	public LinkedList(Boolean isServer)	{
		first = null;
		last = null;
	...
	public String getPrivateMessages(String sender, String recipient)	{
		...
		while (n != null)	{
			...
			if (recipient.equals("All"))	{
				if (n.message.recipient.equals("All"))	{
					countMessage ++;
					conversation += n.message.getPrivateMessages();
				}
				
			}
			else if (((n.message.sender.equals(sender) && n.message.recipient.equals(recipient)) ||
	...
	public String searchMessages(String sender, String recipient, String searchTerm)	{
		String conversation = "";
		Node n = first;
		...
		while (n != null)	{
			if (recipient.equals("All"))	{
				if (n.message.recipient.equals("All"))	{
					...
					// Checks the content of the message for the searched term
					if (n.message.content.contains(searchTerm))
						conversation += n.message.getPrivateMessages();
				}
		...
		if (conversation.isEmpty())	{
			JOptionPane.showMessageDialog(null, "No matches were found for that search, displaying the full conversation");
			conversation = getPrivateMessages(sender, recipient);
		}
	...
	public LinkedList getCurrentUserMessages(String username)	{
		LinkedList copyList = new LinkedList(false);
		Node n = first;
		
		while (n != null)	{
			if ((n.message.sender.equals(username) || n.message.recipient.equals(username) || n.message.recipient.equals("All")))	{
				copyList.add(n.message);
			}
			n = n.next;
		}
